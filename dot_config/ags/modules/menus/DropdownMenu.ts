...
const moveBoxToCursor = (self: Gtk.Container, fixed: boolean) => {
  if (fixed) {
    return
  }
  ...
  globalMousePos.connect("changed", ({ value }) => {
    ...
    // If GDK scaling is applied, then divide width by scaling to get correct coordinates
    const gdkScale = Utils.exec("zsh -c 'echo $GDK_SCALE'")
...
export const DropDownMenu = ({
  name,
  child,
  transition = "crossfade" as Transition,
